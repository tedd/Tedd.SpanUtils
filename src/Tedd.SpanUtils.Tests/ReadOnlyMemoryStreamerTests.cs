...
namespace Tedd.Tests
{
    public class ReadOnlyMemoryStreamerTests
    {
        ...
        [Theory]
        [InlineData(new byte[] { 1, 2, 3, 4, 5 }, 2, SeekOrigin.Begin, 2)]
        [InlineData(new byte[] { 1, 2, 3, 4, 5 }, 1, SeekOrigin.Current, 1)]
        [InlineData(new byte[] { 1, 2, 3, 4, 5 }, 1, SeekOrigin.End, 3)]
        public void Seek_Position(byte[] data, int offset, SeekOrigin origin, int expectedPosition)
        {
            var stream = new ReadOnlyMemoryStreamer(data);
        ...
        public void WriteAndClear_Throws(byte[] buffer)
        {
            var data = new byte[5];
            var stream = new ReadOnlyMemoryStreamer(data);
            ...
            {
                caughtWrite = true;
                Assert.Equal("Memory is read-only.", ex.Message);
            }
            Assert.True(caughtWrite);

            bool caughtClear = false;
            try { stream.Clear(true); }
            catch (ReadOnlyException ex)
            {
                caughtClear = true;
                Assert.Equal("Memory is read-only.", ex.Message);
            }
            Assert.True(caughtClear);
        }
        ...
        {
            var data = new byte[size];
            var writeStream = new MemoryStreamer(data);
