...
# WriteSized() / ReadSized*()
`WriteSized()` and `ReadSized*()` to write and read size to span. These use a simple compression technique where the 2 first bits are used to describe how many bytes are used for size.
...
This means that if you use `span.WriteSized("hello")` then 1 byte is used for size header and 4 bytes are used for the string. While if you do `span.WriteSized(new byte[20_000])` then 3 bytes are used for size header;
