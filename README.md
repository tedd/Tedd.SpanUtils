...
To calculate the number of bytes required for a sized operation programmatically, utilize the `MeasureWriteSize()` method. Alternatively, if you want to know how many bytes the size descriptor will take by examining the first byte directly, simply do `(firstByte>>6)+1`. The result is 1-4.
